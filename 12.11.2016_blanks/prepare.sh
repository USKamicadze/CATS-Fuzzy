...
gcc -w -shared -Wall -O3 $post_lib_source "bitstream.c" "log.c" -o "post_process.so"
...
function prepare_single {
	...
	$afl -o "$work_dir/main" -w -Wno-unused-result "$work_dir/solution.cpp" "$work_dir/checker.cpp" "$work_dir/run.cpp" 
}
x=a/b/c
echo ${x#*/}
